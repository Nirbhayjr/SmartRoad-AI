...
export default function ImageDetectionPage() {
    ...
    // Detect from remote image URL (calls backend /detect/url-image)
    const handleDetectFromUrl = async () => {
        ...
        setLoading(true)
        try {
            const res = await fetch('/detect/url-image', {
                method: 'POST',
            ...
            setError(err.message || 'Failed to detect from URL')
            console.error('URL detect error:', err)
        } finally {
            setLoading(false)
        }
    }

    // Batch dataset (upload zip file or provide zip URL). Calls backend /detect/batch
    const handleBatch = async () => {
        if (!batchFile && (!batchUrlInput || batchUrlInput.trim() === '')) {
            setError('Please provide a zip file or a zip URL for batch mode')
            return
        }
        ...
        try {
            ...
            if (batchFile) {
                ...
                res = await fetch('/detect/batch', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ zip_url: batchUrlInput.trim() })
                })
            }

            if (!res.ok) {
                const txt = await res.text()
                throw new Error(txt || 'Batch server error')
            }
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-6">
                        <div className="bg-gray-800 rounded-lg p-8 border-2 border-gray-700 hover:border-blue-500 transition">
                            ...
                            {/* Detect from URL & Batch Mode */}
                            <div className="mt-6 space-y-4">
                                <div className="bg-gray-900/20 p-4 rounded">
                                    <p className="text-sm text-gray-300 font-semibold mb-2">✔ Detect from URL</p>
                                    <div className="flex gap-2">
                                        ...
                                        <button
                                            ...
                                            className="bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-600 text-white font-semibold py-2 px-4 rounded"
                                        >
                                            {loading ? 'Detecting…' : 'Detect from URL'}
                                        </button>
                                    </div>
                                    <p className="text-xs text-gray-400 mt-2">Provide a direct image link (jpg/png). The frontend will call the backend to fetch and process it.</p>
                                </div>

                                <div className="bg-gray-900/20 p-4 rounded">
                                    <p className="text-sm text-gray-300 font-semibold mb-2">✔ Batch Dataset Mode</p>
                                    <div className="flex flex-col gap-2">
                                        <div className="flex gap-2">
                                            ...
                                            <button
                                                onClick={handleBatch}
                                                disabled={batchLoading || (!batchFile && !batchUrlInput)}
                                                className="bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-600 text-white font-semibold py-2 px-4 rounded"
                                            >
                                        ...
                                        <div className="flex gap-2 items-center">
                                            <input
                                                type="text"
                                                placeholder="Or provide a zip URL (https://...)"
                                                value={batchUrlInput}
                                        ...
                                        <p className="text-xs text-gray-400">Accepted: ZIP of images (.zip). If you provide a zip URL, backend will download, extract and process images server-side.</p>
                                    </div>
                                </div>
                                {batchResult && (
                                    <div className="mt-2 text-sm text-gray-300">
                                        <p>Batch completed: {batchResult.summary?.processed || batchResult.processed || 0} images processed.</p>
                                    </div>
                                )}
                            </div>
                        </div>
